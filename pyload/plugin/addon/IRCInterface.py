...
class IRCInterface(Thread, Addon):
    ...
    def captcha_task(self, task):
        if self.get_config('captcha') and task.is_textual():
    ...
    def event_queue(self, args):
        ps = self.pyload.api.get_queue_data()
    ...
    def event_collector(self, args):
        ps = self.pyload.api.get_collector_data()
        if not ps:
    ...
    def event_del(self, args):
        ...
        if args[0] == "-p":
            ret = self.pyload.api.delete_packages(map(int, args[1:]))
            return ["INFO: Deleted %d packages!" % len(args[1:])]

        elif args[0] == "-l":
            ret = self.pyload.api.del_links(map(int, args[1:]))
    ...
    def event_push(self, args):
        ...
        id = int(args[0])
        try:
            info = self.pyload.api.get_package_info(id)
        except PackageDoesNotExists:
