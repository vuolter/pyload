...
class Link_crypt_ws(Crypter):
    ...
    def prepare(self):
        ...
        self.req.cj.set_cookie("linkcrypt.ws", "language", "en")
    ...
    def is_online(self):
        if "<title>Linkcrypt.ws // Error 404</title>" in self.html:
            self.log_debug("folder doesen't exist anymore")
            return False
        else:
            return True
    ...
    def handle_web_links(self):
        ...
        for idx, weblink_id in enumerate(ids):
            try:
                self.log_debug("Decrypting Web link %d, %s" % (idx + 1, weblink_id))
    ...
    def handle_container(self, type):
        ...
        for line in self.container_html:
            if type in line:
                ...
                self.pyload.api.upload_container( "%s.%s" %(package_name, type), self.load(clink.group(1)))
                return "Found it"
